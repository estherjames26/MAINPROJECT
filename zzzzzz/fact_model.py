...
class TransformerEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, heads, dff, dropout=0.1):
        super().__init__()
        self.att = tf.keras.layers.MultiHeadAttention(num_heads=heads, key_dim=d_model)
...
class TransformerDecoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, heads, dff, dropout=0.1):
        super().__init__()
        self.mha1 = tf.keras.layers.MultiHeadAttention(num_heads=heads, key_dim=d_model)
        self.mha2 = tf.keras.layers.MultiHeadAttention(num_heads=heads, key_dim=d_model)
        self.ffn = tf.keras.Sequential([
            tf.keras.layers.Dense(dff, activation='relu'),
            tf.keras.layers.Dense(d_model)
        ])
